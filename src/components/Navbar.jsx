    ...
    <nav className="sticky top-0 z-50 bg-white border-b shadow-sm">
      <div className="container mx-auto flex items-center justify-between px-4 py-3">
        ...
        <div className="hidden md:flex items-center gap-6 text-gray-700">
          ...
          <Link
            href="/booking"
            className={`hover:text-purple-600 transition ${
              isActive("/booking") ? "text-purple-600 font-semibold" : ""
            }`}
          >
            Rooms
          </Link>
          ...
          {session && (
            <>
              ...
              <Link
                href="/my-listings"
                className={`hover:text-purple-600 transition ${
                  isActive("/my-listings") ? "text-purple-600 font-semibold" : ""
                }`}
              >
                My Listings
              </Link>
      ...
      {open && (
        <div className="md:hidden flex flex-col gap-4 px-4 py-3 border-t bg-white">
          ...
          <Link href="/booking" onClick={() => setOpen(false)}>Rooms</Link>

          {session && (
            <>
              <Link href="/add-room" onClick={() => setOpen(false)}>Add Room</Link>
              <Link href="/my-listings" onClick={() => setOpen(false)}>My Listings</Link>
              <Link href="/my-bookings" onClick={() => setOpen(false)}>My Bookings</Link>
            </>
          )}
